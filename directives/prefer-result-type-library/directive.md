...
description: >
  The JVM implementation for [error-handling](../error-handling/directive.md)'s Result pattern — a concrete library
  choice, not the pattern itself. Split out from the old `default-stack` reference so it can be toggled independently of
  the rest of the JVM stack.
