...
- **split-suites-same-entry-point** — corner cases bloat one suite; several suites hit the same entry point, one concern each
- **extracted-strategy-suite** — the corner cases belong to a strategy that should be extracted, tested in full on its own, and stubbed in the main suite
...
- Does class X exist only to satisfy class Y — remove Y, is X still needed? → X is a candidate for a dedicated-class-suite.
- Is it a util? → util-unit-suite.

Revisit when the suite is bloating with corner cases and losing readability during coverage or refactoring — propose split-suites-same-entry-point or extracted-strategy-suite as a checkpoint.
