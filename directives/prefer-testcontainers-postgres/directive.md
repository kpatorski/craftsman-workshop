...
description: >
  Persistence-touching tests run against a real PostgreSQL in TestContainers, not an in-memory or mocked database. JUnit
  Jupiter is used only to bootstrap the container, even inside a Spock suite. Split out from the old `default-stack`
  reference so it can be toggled independently of the rest of the JVM stack.
