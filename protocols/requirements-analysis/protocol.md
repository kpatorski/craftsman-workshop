...
description: >
  Turns a requirements input into reviewed, written spec files. Every event is collected first, for the whole
  input — cheap, and it gives the Big Picture. After that the work goes one slice (one candidate use case) at a
  time, each carried from rules to a reviewed spec and a direction check before the next begins, so the developer
  can judge the approach on the first complete example instead of after everything has been modelled.
...
steps: [restate-understanding, collect-events, propose-candidate-specs, slice-loop, draw-bounded-contexts,
  ...
  every slice has a reviewed spec, every blocking open question is resolved or left explicitly `status: blocked`,
  and specs/README.md is current
---
...
4. [slice-loop](../slice-loop/protocol.md) — one slice at a time: model it, draft its spec, confirm the direction.
