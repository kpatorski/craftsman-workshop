...
1. Settle the structural decisions a new class forces: which layer it belongs to, its package, whether it is a port or
   an adapter, its name per the layer's convention.
2. Decisions already determined by architecture-profile or the existing project structure are applied silently.
3. **Check whether the aggregate behind this class is already owned by another use case** (a `crud` use case has no
   aggregate — skip this step for it) (domain-design's
