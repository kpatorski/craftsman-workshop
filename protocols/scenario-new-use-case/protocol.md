...
description: >
  A module whose bounded contexts, business rules and communication style (events, ports & adapters) are already decided; event storming is done. The new use case usually mirrors the structure of existing ones, but may introduce its own convention if it genuinely needs to.
