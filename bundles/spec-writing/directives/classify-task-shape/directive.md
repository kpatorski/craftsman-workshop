...
description: >
  ...
  it names the kind of change, not a scenario id belonging to something else. If `implement` (or another tool) is also
  installed, map these shapes to its own scenarios yourself: new-module -> a bootstrap-style scenario, new-behaviour ->
  an add-use-case-style scenario, change-behaviour -> a modify-existing-code-style scenario, coverage-only -> a
  backfill-tests-style scenario.
