...
description: >
  A gap in coverage on code that already works. Runs on its own; the tests land in a separate commit. If the current
  behaviour looks wrong, the gap is noted, not fixed here.
